        ...
        if (options[0].equals(venueCodes.get(i))) {
          ...
          if (options[1].compareTo(SystemDate) < 0) {
            ...
            MessageCli.BOOKING_ATTENDEES_ADJUSTED.printMessage(
                options[3], String.valueOf(minCapacity), venueCapacity.get(i));
            MessageCli.MAKE_BOOKING_SUCCESSFUL.printMessage(
                bookingReference, venueNames.get(i), options[1], String.valueOf(minCapacity));
            ...
            if (bookedDates.contains(options[1])) {
              MessageCli.BOOKING_NOT_MADE_VENUE_ALREADY_BOOKED.printMessage(
                  venueNames.get(i), options[1]);
              return;
              // If the date inputted is not already booking, make the booking, print success
              // message
            } else {
              MessageCli.MAKE_BOOKING_SUCCESSFUL.printMessage(
                  bookingReference, venueNames.get(i), options[1], options[3]);
              bookings.makeBooking(extendedOptions);
              return;
            }
          }
